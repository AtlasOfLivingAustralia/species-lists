...
public class WebService {
   ...
   * necessary).
   *
   * <p>Note: by default, the Accept header will be set to the same content type as the ContentType
   * provided. To override this default behaviour, include an 'Accept' header in the 'customHeaders'
   * parameter.
   *
   ...
   *     true.
   * @param includeUser true to include the userId and email in the request headers and the ALA-Auth
   *     cookie. If using JWTs sends the current user's access token, if false only sends a
   *     ClientCredentials grant token for this apps client id Default = true.
  ...
  public Map<String, Object> get(
    ...
    return send(
        GET, url, params, contentType, null, null, includeApiKey, includeUser, customHeaders);
  }
   ...
   * parameter.
   *
   * <p>The body map will be sent as the JSON body of the request (i.e. use request.getJSON() on the
   * receiving end).
   *
   * @param url The url-encoded url to send the request to
  ...
  Map put(
      ...
      Map customHeaders) {
    return send(
        PUT, url, params, contentType, body, null, includeApiKey, includeUser, customHeaders);
  }
  ...
  public Map<String, Object> post(
      ...
      Map<String, Object> customHeaders) {
    return send(
        POST, url, params, contentType, body, null, includeApiKey, includeUser, customHeaders);
  }
   ...
   * parameter.
   *
   * <p>Each item in the body map will be sent as a separate Part in the Multipart Request. To send
   * the entire map as a single part, you will need too use the format [data: body].
   *
   ...
   * @param params Map of parameters to be appended to the query string. Parameters will be
   *     URL-encoded automatically.
   * @param files List of 0 or more files to be included in the multipart request (note: if files is
   *     null, then the request will NOT be multipart)
   * @param partContentType the desired content type for the request PARTS (the request itself will
   *     always be sent as multipart/form-data). Defaults to application/json. All non-file parts
  ...
  Map postMultipart(
      ...
      Map customHeaders) {
    return send(
        POST, url, params, partContentType, body, files, includeApiKey, includeUser, customHeaders);
  }
   ...
   * @param params Map of parameters to be appended to the query string. Parameters will be
   *     URL-encoded automatically.
   * @param files Map of 0 or more names and files to be included in the multipart request (note: if
   *     files is null, then the request will NOT be multipart)
   * @param partContentType the desired content type for the request PARTS (the request itself will
   *     always be sent as multipart/form-data). Defaults to application/json. All non-file parts
   *     will have the same content type.
   * @param includeApiKey true to include the service's API Key in the request headers (uses
  ...
  public Map delete(
      ...
      Map customHeaders) {
    return send(
        DELETE, url, params, contentType, null, null, includeApiKey, includeUser, customHeaders);
  }
   ...
   *     request will be written to this object
   * @param url The URL of the service to proxy to
   * @param postBody The POST data to send with the proxied request. If it is a Collection, then it
   *     will be converted to JSON, otherwise it will be sent as a String.
   * @param contentType the desired content type for the request. Defaults to application/json.
   * @param includeApiKey true to include the service's API Key in the request headers (uses
   *     property 'service.apiKey'). If using JWTs, instead sends a JWT Bearer tokens Default =
   *     true.
  ...
  private void configureRequestHeaders(
    ...
    // there is no request, so we need to check if requestAttributes exist before trying to get the
    // user details.
  ...
  //                def val = partContentType == ContentType.APPLICATION_JSON && !(value instanceof
  // net.sf.json.JSON) ? value as JSON : value
  ...
  //        entityBuilder.build();
  //    }
  private URLConnection configureConnection(String url, boolean includeApiKey, boolean includeUser)
      throws IOException {
