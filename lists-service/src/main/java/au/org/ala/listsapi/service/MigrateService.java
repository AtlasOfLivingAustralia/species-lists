...
public class MigrateService {
    ...
    private List<SpeciesList> filterExistingLists(List<SpeciesList> lists, boolean doesExist) {
        ...
        Set<String> existingDrUIDs =
                speciesListMongoRepository.findAllByDataResourceUidIsIn(dataResources).stream()
    ...
    private SpeciesList mapListToSpeciesList(Map<String, Object> list) {
        try {
            ...
            speciesList.setIsAuthoritative(
                    list.get("isAuthoritative") instanceof Boolean
            ...
            speciesList.setIsThreatened(
                    list.get("isThreatened") instanceof Boolean
                ...
                speciesList -> {
                    ...
                    if (s3Enabled) {
                        ...
                        try {
                                                ...
                                                try (InputStream is = response.getBody()) {
                                                    ...
                                                    long contentLength =
                                                            response.getHeaders()
                                                                    .getContentLength();
                                                    if (contentLength < 0) {
                                                        // If content length is unknown, we need to
                                                        // read to byte array first
                            ...
                            logger.error(
                                    "Download for " + speciesList.getDataResourceUid() + " failed");
                            logger.error(e.getMessage(), e);

                            if (s3Key != null) {
                                logger.info(
                                        "S3 file will be cleaned up by lifecycle policy after error: {}",
                                        s3Key);
                            }
                        }
                    } else {
                        try {
                            String safeDataResourceUid =
                                    sanitizeForFilename(speciesList.getDataResourceUid());
