...
public class SpeciesListTransformer {
  ...
  public SpeciesListTransformer(
      UserdetailsService userdetailsService, SpeciesListRepository speciesListRepository) {
  ...
  public SpeciesListVersion1 transformToVersion1(SpeciesList speciesList) {
    ...
    version1.setLastMatched(
        speciesList.getLastUpdated()); // Not strictly true but good enough for legacy services
    ...
    if (Boolean.TRUE.equals(legacyLookupUsersEnabled)
  ...
  public SpeciesListItemVersion1 transformToVersion1(SpeciesListItem speciesListItem, int index) {
    ...
    // Generate a pseudo-numeric ID from the String ID (UUID)
    // This is a best-effort conversion for legacy clients expecting Long IDs.
    long fakeId = speciesListItem.getId().hashCode();

    listItemVersion1.setId(fakeId);
    ...
    // Get list details via Repository
    ...
    if (speciesListItem.getProperties() != null) {
      speciesListItem
          ...
          .forEach(
              kvpValue ->
                  kvps.add(
                      new KvpValueVersion1(fixLegacyKeys(kvpValue.getKey()), kvpValue.getValue())));
    }
  ...
  public QueryListItemVersion1 transformToQueryListVersion1(SpeciesListItem speciesListItem) {
    ...
    // Map properties from SpeciesList to SpeciesListVersion1
    queryListItemV1.setId(speciesListItem.getId());
    queryListItemV1.setSpeciesListID(speciesListID);
    queryListItemV1.setDataResourceUid(
        speciesListID); // fallback - attempt to set actual DataResourceUid via lookup, below
    ...
    // Get extra details via Repository lookup
    Optional<SpeciesList> speciesList =
        speciesListRepository.findByIdOrDataResourceUid(speciesListID, speciesListID);
              ...
              kvpValue ->
                  kvps.add(
                      new KvpValueVersion1(
                          replaceKnownKeys(kvpValue.getKey()), kvpValue.getValue())));
    }
