...
public class GraphQLController {
  ...
  private ListSearchContext buildSearchContext(
    ...
    if (hasPrivateFilter != null) {
      filters = addOrUpdateFilter(filters, new Filter("isPrivate", hasPrivateFilter.toString()));
    }

    return ListSearchContext.builder()
        .searchQuery(cleanRawQuery(searchQuery))
  ...
  private PermissionContext determinePermissions(
    ...
    boolean isAdmin =
        isAuthenticated && (authUtils.hasAdminRole(profile) || authUtils.hasInternalScope(profile));
  ...
  private Boolean isPrivateFilterApplied(
    ...
    Boolean hasPrivateFilterOrFlag = getPrivateFilterOrFlag(filters, isPrivate);
  ...
  public Page<Release> listReleases(
    ...
    return releaseRepository.findBySpeciesListID(speciesListID, pageable);
  }
  ...
  public SpeciesList getSpeciesListMetadata(
    ...
    if (speciesListOptional.isPresent()) {
      ...
      if (Boolean.TRUE.equals(speciesList.getIsPrivate())
  ...
  public SpeciesList addField(
    ...
    Optional<SpeciesList> optionalSpeciesList =
        speciesListRepository.findByIdOrDataResourceUid(id, id);
    ...
    if (toUpdate.getFieldList() == null) {
      toUpdate.setFieldList(new ArrayList<>());
    }
    toUpdate.getFieldList().add(fieldName);

    if (StringUtils.isNotEmpty(fieldValue)) {
      int batchSize = MAX_LIST_ENTRIES; // 10000
      String lastId = null;
        ...
        if (lastId == null) {
          items =
              speciesListItemRepository.findFirstBatch(
                  toUpdate.getId(), PageRequest.of(0, batchSize));
        } else {
          items =
    ...
    return speciesListRepository.save(toUpdate);
  }
  ...
  public SpeciesListItem updateSpeciesListItem(
    ...
    Optional<SpeciesListItem> optionalSpeciesListItem =
        speciesListItemRepository.findById(inputSpeciesListItem.getId());
    if (optionalSpeciesListItem.isEmpty()) {
      return null;
    }

    Optional<SpeciesList> optionalSpeciesList =
        speciesListRepository.findById(inputSpeciesListItem.getSpeciesListID());
    ...
    // save item again with classification
    speciesListItemRepository.save(speciesListItem);
  ...
  private SpeciesListItem updateItem(
    ...
    if (inputSpeciesListItem.getProperties() != null) {
    ...
    return speciesListItemRepository.save(speciesListItem);
  }
  ...
  public SpeciesListItem addSpeciesListItem(
    ...
    // update distinct match count
    speciesList.setDistinctMatchCount(taxonService.getDistinctTaxaCount(speciesList.getId()));
    speciesList.setRowCount(
        (speciesList.getRowCount() != null ? speciesList.getRowCount() : 0) + 1);
    speciesListRepository.save(speciesList);
  ...
  private SpeciesList updateLastUpdated(SpeciesList speciesList, Principal principal) {
    ...
    return speciesListRepository.save(speciesList);
  }
  ...
  public SpeciesListItem removeSpeciesListItem(
    ...
    Optional<SpeciesListItem> optionalSpeciesListItem = speciesListItemRepository.findById(id);
    if (optionalSpeciesListItem.isEmpty()) {
      return null;
    }

    Optional<SpeciesList> optionalSpeciesList =
        speciesListRepository.findById(optionalSpeciesListItem.get().getSpeciesListID());
    if (optionalSpeciesList.isEmpty()) {
      return null;
    }
    ...
    speciesListItemRepository.deleteById(id);

    // update distinct match count
    speciesList.setDistinctMatchCount(taxonService.getDistinctTaxaCount(speciesList.getId()));
    int rowCount = (speciesList.getRowCount() != null ? speciesList.getRowCount() : 0);
    speciesList.setRowCount(Math.max(0, rowCount - 1));
    speciesListRepository.save(speciesList);
    ...
    if (authUtils.isAuthorized(toUpdate, principal)) {
      ...
      try {
        if (Boolean.FALSE.equals(toUpdate.getIsPrivate()) // saved list is public
            || Boolean.TRUE.equals(
                toUpdate.getIsAuthoritative()) // saved list is authoritative (private or public)
        ...
        logger.error(
            "Error while setting metadata for species list: " + id + " - " + e.getMessage(), e);
      }
  ...
  public Page<SpeciesListItem> filterSpeciesList(
    ...
    SingleListSearchContext searchContext =
        buildSingleListSearchContext(speciesList, searchQuery, filters, sort, dir, principal);

    // Delegate to service for the actual search
    return searchHelperService.searchSingleSpeciesList(searchContext, PageRequest.of(page, size));
  }
  ...
  public List<Facet> facetSpeciesList(
    ...
    // Build search context for the specific list
    SingleListSearchContext searchContext =
        buildSingleListSearchContext(speciesList, searchQuery, filters, null, null, principal);

    // Use provided facet fields or default to list's configured facets
    List<String> effectiveFacetFields =
        (facetFields != null && !facetFields.isEmpty()) ? facetFields : speciesList.getFacetList();

    // Delegate to service for facet aggregation
    return searchHelperService.getFacetsForSingleSpeciesList(searchContext, effectiveFacetFields);
  }
  ...
  private SingleListSearchContext buildSingleListSearchContext(
    ...
    AlaUserProfile profile = authUtils.getUserProfile(principal);
    boolean isAdmin =
        profile != null && (authUtils.hasAdminRole(profile) || authUtils.hasInternalScope(profile));
