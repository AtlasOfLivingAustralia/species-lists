...
public class GraphQLController {
    ...
    public Page<SpeciesList> lists(
        ...
        ListSearchContext searchContext =
                buildSearchContext(searchQuery, isPrivate, filters, userId, sort, dir, principal);

        // Delegate to service for the actual search
        return searchHelperService.searchSpeciesLists(searchContext, PageRequest.of(page, size));
    }
    ...
    public List<Facet> facetSpeciesLists(
        ...
        // Build search context with permission checks
        ListSearchContext searchContext =
                buildSearchContext(searchQuery, isPrivate, filters, userId, null, null, principal);
    ...
    private ListSearchContext buildSearchContext(
        ...
        Boolean hasPrivateFilter =
                isPrivateFilterApplied(filters, isPrivate, permissions.isAdmin(), isMySpeciesList);
        if (hasPrivateFilter != null) {
            filters =
                    ElasticUtils.addOrUpdateFilter(
                            filters, new Filter("isPrivate", hasPrivateFilter.toString()));
        }
    ...
    private PermissionContext determinePermissions(
            String userId, List<Filter> filters, AlaUserProfile profile) {
    ...
    private Boolean getPrivateFilterOrFlag(List<Filter> filters, Boolean isPrivate) {
        boolean isPrivateInFilters =
                filters != null && filters.stream().anyMatch(f -> "isPrivate".equals(f.getKey()));
    ...
    /**
     * Determines if the isPrivate filter is applied based on the provided filters and isPrivate
     * argument.
     *
     * <p><b>Special logic for admins:</b> If the user is an admin and no private filter or flag is
     * set, this method returns {@code null}. This signals downstream queries (such as Elasticsearch
     * queries) to not filter on the {@code isPrivate} field at all, allowing admins to see both
     * private and public lists. For non-admins, or if a private filter/flag is set, this method
     * returns {@code true} if a private filter or flag is set, or {@code false} otherwise. This
     * ensures that non-admin users are always subject to privacy filtering, while admins have
     * unrestricted access unless a filter is explicitly applied.
     *
     * <p><b>Downstream effect:</b> Returning {@code null} for admins means that the query will not
     * include any constraint on the {@code isPrivate} field, so all lists (public and private) are
     * visible to admins.
     *
     ...
     * @return {@code true} if the isPrivate filter is applied, {@code false} if not, or {@code
     *     null} for admins with no private filter (no filtering).
     */
    @Nullable
    private Boolean isPrivateFilterApplied(
            List<Filter> filters, Boolean isPrivate, Boolean isAdmin, Boolean isMySpeciesList) {
        boolean defaultValue = false;
        Boolean hasPrivateFilterOrFlag =
                getPrivateFilterOrFlag(filters, isPrivate); // can be null, true or false
        // boolean isPrivateSpecified = isPrivate != null && isPrivate;
        boolean isAdminSpecified = isAdmin != null && isAdmin;
        // If the user is an admin and no private filter or flag is set, return null (no filtering
        // on isPrivate).
    ...
    public GraphQLError handle(
            @NonNull Throwable ex, @NonNull DataFetchingEnvironment environment) {
    ...
    public SpeciesList addField(
        ...
        Optional<SpeciesList> optionalSpeciesList =
                speciesListMongoRepository.findByIdOrDataResourceUid(id, id);
            ...
            while (!finished) {
                List<SpeciesListItem> items =
                        speciesListItemMongoRepository.findNextBatch(
                                toUpdate.getId(), lastId, PageRequest.of(0, batchSize));
            ...
            for (SpeciesListItem item : items) {
                Optional<KeyValue> kv =
                        item.getProperties().stream()
                                .filter(k -> k.getKey().equals(oldName))
                                .findFirst();
    ...
    public SpeciesList removeField(
        ...
        while (!finished) {
            ...
            for (SpeciesListItem item : items) {
                Optional<KeyValue> kv =
                        item.getProperties().stream()
                                .filter(k -> k.getKey().equals(fieldName))
                                .findFirst();
    ...
    public SpeciesListItem updateSpeciesListItem(
        ...
        if (!authUtils.isAuthorized(optionalSpeciesList.get(), principal)) {
            logger.info(
                    "User not authorized to modify access list: "
                            + optionalSpeciesList.get().getId());
            throw new AccessDeniedException("You dont have access to this list");
        }
    ...
    private void reindex(SpeciesListItem speciesListItem, SpeciesList speciesList) {
        ...
        SpeciesListIndex speciesListIndex =
                new SpeciesListIndex(
                        ...
                        speciesList.getIsThreatened() != null
                                ? speciesList.getIsThreatened()
                                : false,
                        speciesList.getIsInvasive() != null ? speciesList.getIsInvasive() : false,
                        StringUtils.isNotEmpty(speciesList.getRegion())
                                || StringUtils.isNotEmpty(speciesList.getWkt()),
                        speciesList.getOwner(),
                        speciesList.getEditors(),
                        speciesList.getTags(),
                        speciesList.getDateCreated() != null
                                ? speciesList.getDateCreated().toString()
                                : null,
                        speciesList.getLastUpdated() != null
                                ? speciesList.getLastUpdated().toString()
                                : null,
    ...
    public SpeciesListItem removeSpeciesListItem(
        ...
        Optional<SpeciesListItem> optionalSpeciesListItem =
                speciesListItemMongoRepository.findById(id);
        if (optionalSpeciesListItem.isEmpty()) {
            return null;
        }

        Optional<SpeciesList> optionalSpeciesList =
                speciesListMongoRepository.findById(
                        optionalSpeciesListItem.get().getSpeciesListID());
        if (optionalSpeciesList.isEmpty()) {
            return null;
        }
    ...
    public SpeciesList updateMetadata(
        ...
        if (authUtils.isAuthorized(toUpdate, principal)) {
            ...
            if (title != null && !title.equalsIgnoreCase(toUpdate.getTitle())
                    || description != null
                            && !description.equalsIgnoreCase(toUpdate.getDescription())
                    ...
                    || isAuthoritative != null
                            && !isAuthoritative.equals(toUpdate.getIsAuthoritative())
            ...
            boolean previousIsPrivate =
                    toUpdate.getIsPrivate() != null ? toUpdate.getIsPrivate() : false;
            ...
            try {
                if (Boolean.FALSE.equals(toUpdate.getIsPrivate()) // saved list is public
                        || Boolean.TRUE.equals(
                                toUpdate.getIsAuthoritative()) // saved list is authoritative
                        // (private or public)
                        || (!previousIsPrivate
                                && Boolean.TRUE.equals(isPrivate)) // was public, now private
                ) {
                    metadataService.setMeta(toUpdate);
                }
            } catch (Exception e) {
                logger.error(
                        "Error while setting metadata for species list: "
                                + id
                                + " - "
                                + e.getMessage(),
                        e);
            ...
            if (reindexRequired) {
                logger.debug(
                        "Reindexing list {} after metadata update. isPrivate changed to: {}",
                        updatedList.getId(),
                        updatedList.getIsPrivate());
    ...
    public Page<SpeciesListItem> filterSpeciesList(
        ...
        SingleListSearchContext searchContext =
                buildSingleListSearchContext(
                        speciesList, searchQuery, filters, sort, dir, principal);

        // Delegate to service for the actual search
        return searchHelperService.searchSingleSpeciesList(
                searchContext, PageRequest.of(page, size));
    }
    ...
    public List<Image> getTaxonImages(
            @Argument String taxonID, @Argument Integer page, @Argument Integer size)
            throws Exception {
        // get taxon image from BIE
