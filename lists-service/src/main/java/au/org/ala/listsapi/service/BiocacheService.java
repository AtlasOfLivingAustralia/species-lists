...
public class BiocacheService {
  ...
  private <T> Stream<List<T>> batches(List<T> source) {
    ...
    return IntStream.range(0, fullChunks + 1)
        .mapToObj(
            n ->
                source.subList(
                    n * maxBooleanClause, n == fullChunks ? size : (n + 1) * maxBooleanClause));
  }
  ...
  public String getQidForSpeciesList(String speciesListId) throws Exception {
    ...
    do {
      ...
      batch =
          speciesListItemRepository.findBySpeciesListIDOrderByIdAsc(
              speciesListId, PageRequest.of(page, pageSize));
    ...
    String finalQuery = batchQueries.stream().collect(Collectors.joining(" OR "));
