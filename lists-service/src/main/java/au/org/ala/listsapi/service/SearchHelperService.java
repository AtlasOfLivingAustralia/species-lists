 ...
 * write operations for species list items, interacting with both MongoDB and Elasticsearch.
 */
@Service
public class SearchHelperService {
    private static final org.slf4j.Logger logger =
            org.slf4j.LoggerFactory.getLogger(SearchHelperService.class);
    ...
    public BulkWriteResult speciesListItemsBulkUpdate(
            List<SpeciesListItem> items, List<String> keys) {
        BulkOperations bulkOps =
                mongoTemplate.bulkOps(BulkOperations.BulkMode.UNORDERED, SpeciesListItem.class);
    ...
    /**
     * Fetches species list items based on GUIDs and optional species list IDs. Supports pagination
     * and filtering pof public/private lists based on user roles.
     *
     * @param guids
    ...
    public List<SpeciesListItem> fetchSpeciesListItems(
                ...
                q ->
                        q.bool(
                                bq -> {
                                    ...
                                    // If the user is not an admin or doesn't have internal scope,
                                    // only query their private lists, and all other public lists
                                    if (!authUtils.isAuthenticated(principal)) {
                                        logger.debug(
                                                "Filtering for public lists only (user not authenticated)");
                                        bq.filter(
                                                f ->
                                                        f.term(
                                                                t ->
                                        ...
                                        logger.debug(
                                                "Filtering for private lists only (non-admin/non-internal users)");
                                        bq.filter(
                                                f ->
                                                        f.bool(
                                                                b ->
                                    ...
                                    // If user is admin or has internal scope, no filters applied
                                    // (can see all lists)
        ...
        return ElasticUtils.convertList(
                (List<SpeciesListIndex>) SearchHitSupport.unwrapSearchHits(results));
    }

    /**
     * Fetches species list items based on species list IDs and optional search query that can be
     * restricted to specific fields. Supports pagination and filtering of public/private lists
     * based on user roles.
     *
     * @param speciesListIDs
     * @param searchQuery
    ...
    public List<SpeciesListItem> fetchSpeciesListItems(
            ...
            @Nullable
                    Boolean noNulls, // currently not implemented -> delete if testing shows this is
            // not being called
        ...
        Page<SpeciesListItem> itemsPage =
                speciesListItemMongoRepository.findNextBatch(validListIDs, query, pageable);
        return itemsPage.getContent();
    }

    /**
     * Finds common keys across multiple SpeciesList objects. This method is useful for identifying
     * shared attributes
     *
     ...
     * Documents can be filtered by attributes such as isAuthoritative, isThreatened, and
     * isInvasive. The search also respects user roles, ensuring that private lists are only visible
     * to their owners or to users with admin privileges.
     *
    ...
    public Page<SpeciesList> searchDocuments(
        ...
        if (speciesListQuery.getIsAuthoritative() != null) {
            query.addCriteria(
                    Criteria.where("isAuthoritative").is(speciesListQuery.getIsAuthoritative()));
        }
        if (speciesListQuery.getIsThreatened() != null) {
            query.addCriteria(
                    Criteria.where("isThreatened").is(speciesListQuery.getIsThreatened()));
        }
        ...
        if (speciesListQuery.getDataResourceUid() != null) {
            if (speciesListQuery.getDataResourceUid().contains(",")) {
                List<String> dataResourceUids =
                        Arrays.asList(speciesListQuery.getDataResourceUid().split(","));
                query.addCriteria(Criteria.where("dataResourceUid").in(dataResourceUids));
            } else {
                query.addCriteria(
                        Criteria.where("dataResourceUid")
    ...
    /**
     * Builds a Criteria object that enforces document access based on user roles and additional
     * search criteria.
     *
     * @param userId
     * @param isAdmin
    ...
    private Criteria buildDocumentAccessCriteria(
            String userId, Boolean isAdmin, Criteria additionalCriteria) {
                    ...
                    new Criteria()
                            .orOperator(
                                    Criteria.where("owner")
                                            .is(userId), // All documents owned by user
                                    Criteria.where("isPrivate")
                                            .is(false) // Public documents from anyone
                                    );
    ...
    public Page<SpeciesList> searchSpeciesLists(ListSearchContext context, Pageable pageable) {
        ...
        // Execute search
        SearchHits<SpeciesListIndex> results =
                elasticsearchOperations.search(builder.build(), SpeciesListIndex.class);
    ...
    private void buildListSearchQuery(ListSearchContext context, BoolQuery.Builder bq) {
    ...
    private void applyPermissionFilters(ListSearchContext context, BoolQuery.Builder bq) {
        ...
        builder.withAggregation(
                ...
                Aggregation.of(
                        a ->
                                a.terms(
                                                ta ->
                                                        ta.field(SPECIES_LIST_ID + ".keyword")
    ...
    private void addFacetAggregations(NativeQueryBuilder builder) {
        ...
        for (String field : facetFields) {
            String esField = booleanFields.contains(field) ? field : field + ".keyword";

            builder.withAggregation(
                    field,
                    Aggregation.of(
                            a ->
                                    a.terms(ta -> ta.field(esField).size(50))
                                            .aggregations(
                                                    "distinct_list_count",
                                                    Aggregation.of(
                                                            ca ->
                ...
                .collect(
                        Collectors.toMap(
                                ...
                                b -> {
                                    ...
                                    return (Double.isInfinite(score) || Double.isNaN(score))
                                            ? 0.0
                                            : score;
                                }));
    }
    ...
    private List<SpeciesList> fetchAndEnrichLists(
            Map<String, Long> counts, Map<String, Double> scores, ListSearchContext context) {

        if (counts.isEmpty()) return Collections.emptyList();

        List<SpeciesList> lists = new ArrayList<>();
        speciesListMongoRepository
                .findAllById(counts.keySet())
                .forEach(
                        list -> {
    ...
    private Comparator<SpeciesList> buildComparator(
            String sort, String dir, Map<String, Double> scores) {
            ...
            case "relevance":
                comparator =
                        Comparator.comparing(
                                        (SpeciesList list) ->
                                                scores.getOrDefault(list.getId(), 0.0))
                                .thenComparing(
                                        list -> list.getTitle() != null ? list.getTitle() : "",
                                        String.CASE_INSENSITIVE_ORDER);
                return ascending ? comparator : comparator.reversed();

            case "title":
                comparator =
                        Comparator.comparing(SpeciesList::getTitle, String.CASE_INSENSITIVE_ORDER);
                break;
    ...
    private Page<SpeciesList> paginateResults(
            List<SpeciesList> lists, Pageable pageable, long total) {
            ...
            if (aggregate.isSterms()) {
                ...
                aggregate
                        ...
                        .forEach(
                                bucket -> {
                                    long distinctCount =
                                            getDistinctListCount(bucket.aggregations());
                                    facet.getCounts()
                                            .add(
                                                    new FacetCount(
    ...
    /**
     * Builds the base query for a specific list (list ID + search query only) Used for facet
     * aggregations where we want all items before filtering
     */
    private void buildBaseListQuery(SingleListSearchContext context, BoolQuery.Builder bq) {
    ...
    private void buildSingleListQuery(SingleListSearchContext context, BoolQuery.Builder bq) {
        ...
        if (context.getFilters() != null && !context.getFilters().isEmpty()) {
            Map<String, List<Filter>> filtersByKey =
                    context.getFilters().stream().collect(Collectors.groupingBy(Filter::getKey));
    ...
    private void applyFilterGroup(String field, List<Filter> filters, BoolQuery.Builder bq) {
        List<String> values =
                filters.stream().map(Filter::getValue).distinct().collect(Collectors.toList());
    ...
    private BoolQuery.Builder buildOrQuery(
            ...
            BiFunction<
                            ...
                            ObjectBuilder<co.elastic.clients.elasticsearch._types.query_dsl.Query>>
                    termQueryFactory) {
    ...
    private void applySorting(NativeQueryBuilder builder, String sort, String dir) {
        ...
        builder.withSort(s -> s.field(new FieldSort.Builder().field(sort).order(order).build()));
    }

    /** Adds facet aggregations for the specified fields */
    private void addSingleListFacetAggregations(
            NativeQueryBuilder builder, List<String> facetFields) {
    ...
    private void addPropertyAggregations(NativeQueryBuilder builder) {
        builder.withAggregation(
                ...
                Aggregation.of(
                        a ->
                                a.nested(na -> na.path("properties"))
                                        .aggregations(
                                                "key_counts",
                                                sa ->
                                                        sa.terms(
    ...
    private List<Facet> processSingleListFacets(
        ...
        for (String field : allFields) {
            ElasticsearchAggregation aggResult =
    ...
    private Facet createFacetFromTerms(String fieldName, List<StringTermsBucket> buckets) {
        ...
        for (StringTermsBucket bucket : buckets) {
            facet.getCounts().add(new FacetCount(bucket.key().stringValue(), bucket.docCount()));
        }
    ...
    private List<Facet> processPropertyFacets(
            ElasticsearchAggregations agg, SingleListSearchContext context) {
    ...
    private List<String> extractPropertyKeys(ElasticsearchAggregations agg) {
        ...
        ElasticsearchAggregation propertiesAgg =
                agg.aggregations().stream()
    ...
    private Facet extractPropertyValueFacet(
            String propertyKey, SearchHits<SpeciesListIndex> results) {
        ElasticsearchAggregations agg = (ElasticsearchAggregations) results.getAggregations();
        if (agg == null) {
            return null;
        }

        ElasticsearchAggregation keyValueAgg =
                agg.aggregations().stream()
