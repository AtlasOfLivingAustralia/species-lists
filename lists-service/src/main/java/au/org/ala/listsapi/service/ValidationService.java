...
public class ValidationService {
    ...
    private void init() throws IOException {
        ...
        try (InputStream inputStream = constraintsFile.getInputStream()) {
            ...
            throw new RuntimeException(
                    "Failed to read or parse constraints file from: "
                            + constraintsFile.getDescription(),
                    e);
        }
    }
    ...
    public boolean isValueValid(ConstraintType constraintType, String value) {
        ...
        return list.stream().filter(elm -> elm.getValue().equals(value)).findAny().orElse(null)
                != null;
    }
