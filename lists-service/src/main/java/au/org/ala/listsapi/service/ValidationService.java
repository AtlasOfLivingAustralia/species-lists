...
public class ValidationService {
    ...
    try (InputStream inputStream = constraintsFile.getInputStream()) {
      ...
      constraints =
          objectMapper.readValue(
              inputStream, new TypeReference<HashMap<String, List<ConstraintListItem>>>() {});
      ...
      throw new RuntimeException(
          "Failed to read or parse constraints file from: " + constraintsFile.getDescription(), e);
    }
  }
  ...
  public boolean isValueValid(ConstraintType constraintType, String value) {
    ...
    return list.stream().filter(elm -> elm.getValue().equals(value)).findAny().orElse(null) != null;
  }

  public boolean isListValid(InputSpeciesList speciesList) {
    // check that the supplied list type, region and license is valid
    return (isValueValid(ConstraintType.listType, speciesList.getListType())
