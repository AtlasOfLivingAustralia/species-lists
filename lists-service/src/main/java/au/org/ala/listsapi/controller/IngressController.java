...
public class IngressController {
  ...
  public ResponseEntity<Object> handleFileUpload(
      @RequestPart("file") MultipartFile file, @AuthenticationPrincipal AlaUserProfile profile) {
    ...
    try {
      logger.info(
          s3Enabled ? "Upload to S3 started..." : "Upload to temporary local storage started...");
  ...
  public ResponseEntity<Object> ingest(
      @Parameter(
              description =
                  "Value should be the `localFile` property returned from the `/upload` endpoint")
          @RequestParam("file")
          String fileIdentifier,
      InputSpeciesList speciesList,
      @Parameter(
              description = "Spceies list metadata. See the `InputSpeciesList` model for details.")
  ...
  private ResponseEntity<Object> startAsyncTaskIfNotBusy(String name, Runnable runnable) {
    if (asyncTask == null || asyncTask.isDone()) {
      ...
      return new ResponseEntity<>("Task " + taskName + " is already running.", HttpStatus.CONFLICT);
    }
  }
}
