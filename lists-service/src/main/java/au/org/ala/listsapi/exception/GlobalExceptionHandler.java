...
public class GlobalExceptionHandler {
    ...
    public ErrorResponse handleConstraintViolation(ConstraintViolationException ex) {
        ...
        List<String> errors =
                ex.getConstraintViolations().stream()
                        .map(
                                violation -> {
                                    // Extract just the parameter name (e.g., "getUsers.pageSize" ->
                                    // "pageSize")
                                    ...
                                    return String.format(
                                            "%s: %s (received: %s)",
                                            paramName, message, invalidValue);
                                })
    ...
    public ErrorResponse handleMissingParams(MissingServletRequestParameterException ex) {
        String message = String.format("%s: Required parameter is missing", ex.getParameterName());
