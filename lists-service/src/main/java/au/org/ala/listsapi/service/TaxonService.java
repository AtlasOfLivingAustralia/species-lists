...
public class TaxonService {
                                                    ...
                                                    speciesList -> {
                                                        try {
                                                            long distinctMatchCount =
                                                                    taxonMatchDataset(
                                                                            speciesList.getId());
                                                            speciesList.setDistinctMatchCount(
                                                                    distinctMatchCount);
                                                            speciesListMongoRepository.save(
                                                                    speciesList);
                                                            reindex(speciesList.getId());
                                                            int processed =
                                                                    processedCount
                                                            ...
                                                            // Update progress service periodically
                                                            // (every 10 lists or at completion)
                                                            // This prevents jumpy UI updates from
                                                            // parallel processing
                                                            if (processed % 10 == 0
                                                                    || processed
                                                                            == totalLists.get()) {
    ...
    private void bulkIndexSafe(List<IndexQuery> updateList, SpeciesList list) {
        ...
        try {
            ...
            try {
                Optional<IndexQuery> failedItem =
                        updateList.stream()
                                .filter(item -> failedIds.contains(item.getId()))
                                .findFirst();
    ...
    private SpeciesListIndex listItemToIndex(
            SpeciesList speciesList, SpeciesListItem speciesListItem) {
    ...
    public void reindex(String speciesListID) {
        ...
        while (!finished) {
            ...
            if (lastId == null) {
                speciesListItems =
                        speciesListItemMongoRepository.findFirstBatch(
                                speciesList.getId(), PageRequest.of(0, batchSize));
            } else {
                speciesListItems =
                        speciesListItemMongoRepository.findNextBatchAfter(
            ...
            logger.info(
                    "[{}|reindex] Fetched {} items in {} ms",
                    speciesListID,
                    speciesListItems.size(),
                    elapsed / 1_000_000);
    ...
    public long taxonMatchDataset(String speciesListID) {
        ...
        long findByIdElapsed = (System.nanoTime() - findByIdStart) / 1000000;
        logger.info(
                "[{}|taxonMatch] Taxon match find by ID OR UID {}ms",
                speciesListID,
                findByIdElapsed);
        ...
        logger.info(
                "[{}|taxonMatch] Reset ingest progress {}ms", speciesListID, resetProgressElapsed);
        ...
        while (!finished) {
            ...
            if (items.isEmpty()) {
                ...
                try {
                    ...
                    searchHelperService.speciesListItemsBulkUpdate(
                            items, List.of("classification"));
                    ...
                    long updateProgressElapsed =
                            (System.nanoTime() - updatedProgressStart) / 1000000;
                    logger.info(
                            "[{}|taxonMatch] Update mongo progress {}ms",
                            speciesListID,
                    ...
                    items.forEach(
                            speciesListItem -> {
                                ...
                                if (classification != null
                                        && classification.getTaxonConceptID() != null) {
    ...
    public Classification lookupTaxon(SpeciesListItem item) {
        try {
            ...
            Optional<SpeciesList> optionalSpeciesList =
                    speciesListMongoRepository.findById(item.getSpeciesListID());
    ...
    /**
     * Multi-threaded classification lookup using ALANameUsageMatchServiceClient. This method uses
     * the bulk matchAll() API and processes multiple batches in parallel with rate limiting to
    ...
    public List<Classification> lookupTaxa(List<SpeciesListItem> items, SpeciesList speciesList) {
        ...
        try {
            ...
            logger.info(
                    "[{}|taxonMatch] Built {} name searches", speciesListID, nameSearches.size());
                        ...
                        CompletableFuture.supplyAsync(
                                () -> {
                                    try {
                                        ...
                                        rateLimiter.acquire();
                                        long semaphoreWaitTime =
                                                (System.nanoTime() - semaphoreWaitStart)
                                                        / 1_000_000;
                                        ...
                                        try {
                                            ...
                                            List<NameUsageMatch> results =
                                                    nameMatchService.matchAll(batch);
                                            long batchElapsed =
                                                    (System.nanoTime() - batchStart) / 1_000_000;

                                            // Warn if batch took unusually long (possible
                                            // throttling or GC)
            ...
            logger.error(
                    "[{}|taxonMatch] Exception during bulk lookup: {}",
                    speciesListID,
                    e.getMessage(),
                    e);
    ...
    private List<NameUsageMatch> fillMissingMatches(
            List<NameUsageMatch> results, int expectedSize) {
        if (results == null) {
    ...
    /**
     * Build a NameSearch object from a SpeciesListItem This method replicates the logic from
     * ColumnMatchingService.buildSearch()
     *
     * @param item The species list item
     * @param speciesList The parent species list
    ...
    public long getDistinctTaxaCount(String speciesListID) {
        ...
        try {
            ...
            logger.error(
                    "Error fetching distinct taxonConceptID count for speciesListID: "
                            + speciesListID,
                    e);
        }
