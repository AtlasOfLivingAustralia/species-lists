...
public class LegacyController {
  ...
  public ResponseEntity<Object> speciesListSearch(
      ...
      @Parameter(description = "Query string (q)") @Nullable @RequestParam(name = "q") String query,
    ...
    try {
      ...
      fixLegacyBooleanSyntax(
          isAuthoritative, isThreatened, isInvasive, isSDS, isBIE, druid, speciesListQuery);
  ...
  public ResponseEntity<Object> speciesListItemsSearch(
      ...
      @Parameter(description = "Sort field")
          ...
          @RequestParam(name = "sort", defaultValue = "speciesListID", required = false)
          String sort,
      @Parameter(description = "Sort direction")
          @Schema(allowableValues = {"asc", "desc"})
          @RequestParam(name = "order", defaultValue = "asc")
    ...
    try {
      ...
      Pageable paging =
          PageRequest.of(
              0, 10000); // we want all matching lists, paging will be applied to items later
      RESTSpeciesListQuery speciesListQuery = new RESTSpeciesListQuery();
      fixLegacyBooleanSyntax(
          isAuthoritative, isThreatened, isInvasive, isSDS, isBIE, druid, speciesListQuery);
      ...
      String speciesListIDs =
          speciesLists.getContent().stream()
              .map(
                  list ->
                      list.getDataResourceUid() != null ? list.getDataResourceUid() : list.getId())
  ...
  @Operation(
      tags = "REST v1",
      summary = "Get species list items for a given species list ID",
      deprecated = true)
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
  ...
  public ResponseEntity<Object> speciesListItems(
      @Parameter(
              name = "druid",
              description =
                  "The data resource id (or speciesListID) or comma separated ids to identify list(s) to return list items for e.g. '/v1/speciesListItems/dr123,dr456,dr789'",
  ...
  private ResponseEntity<Object> getLegacySpeciesListItems(
    ...
    try {
      speciesListItems =
          searchHelperService.fetchSpeciesListItems(
              speciesListIDs, searchQuery, fields, nonulls, page, pageSize, sort, dir, principal);
    } catch (Exception e) {
      ...
      return ResponseEntity.status(HttpStatus.BAD_REQUEST)
          ...
          .body(
              new ErrorResponse(
                  HttpStatus.BAD_REQUEST.name(), e.getMessage(), HttpStatus.BAD_REQUEST.value()));
    }
  ...
  public ResponseEntity<Object> speciesListItemsForGuid(
    ...
    int pageSizeVal =
        Math.max((pageSize != null ? pageSize : 999999), 1); // Ensure pageSize is at least 1

    String inputGuids =
        (StringUtils.isNotBlank(guid) ? guid : (StringUtils.isNotBlank(guids) ? guids : ""));
    int pageIndex = (pageVal - 1); // spring data pageable is zero based
    String searchQuery =
        (inputGuids != null) ? inputGuids.replaceAll(",", "|") : null; // convert to regex OR
  ...
  private static int[] calculatePageAndSize(@Nullable Integer offset, @Nullable Integer max) {
    int page =
        ((offset != null ? offset : 0)
            / (max != null ? max : 10)); // was + 1 for 1-based page, but Pageable is zero-based
