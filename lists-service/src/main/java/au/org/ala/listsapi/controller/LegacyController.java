...
/**
 * Controller for legacy API endpoints (/v1/**), that are deprecated but still supported for
...
public class LegacyController {
    ...
    @Autowired protected SpeciesListLegacyService legacyService;
    ...
    @Operation(
            tags = "REST v1",
            summary = "Get species list metadata for all lists",
            deprecated = true)
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
                description = "Species lists found",
                content =
                        @Content(
                                mediaType = MediaType.APPLICATION_JSON_VALUE,
                                schema = @Schema(implementation = SpeciesListPageVersion1.class))),
        @ApiResponse(
                responseCode = "400",
    ...
    public ResponseEntity<Object> speciesListSearch(
            ...
            @Parameter(description = "The data resource id (or speciesListID)", example = "dr656")
                    @Nullable
                    @RequestParam(name = "druid")
                    String druid,
            @Parameter(description = "Query string (q)") @Nullable @RequestParam(name = "q")
                    String query,
            @Parameter(description = "Sort field")
                    ...
                    @RequestParam(name = "sort", defaultValue = "listName", required = false)
                    String sort,
            ...
            @RequestParam(name = "offset", defaultValue = "0", required = false) @Max(9990)
                    int offset,
        ...
        try {
            ...
            query =
                    StringUtils.isNotBlank(query)
                            ...
                            : ".*"; // regex for all if blank
            Page<SpeciesList> results =
                    searchHelperService.searchDocuments(
                            convertedSpeciesListQuery, userId, isAdmin, query, paging);
    ...
    private String fixSortField(String sort) {
        ...
        Map<String, String> sortFieldMapping =
                Map.of(
                        "count", "rowCount",
                        "listName", "title",
        ...
        return sortFieldMapping.getOrDefault(
                sort, sort); // Default to the provided field if no mapping exists
    }

    /**
     * Fix legacy boolean syntax from the old API (eq:true, eq:false) to just true/false. and add to
     * the speciesListQuery object.
     *
     ...
     */
    private static void fixLegacyBooleanSyntax(
        ...
        if (StringUtils.isNotBlank(isAuthoritative)) {
            speciesListQuery.setIsAuthoritative(
                    isAuthoritative.replaceAll("eq:", "")); // eq:true to true, etc.
        }

        if (StringUtils.isNotBlank(isThreatened)) {
            speciesListQuery.setIsThreatened(
                    isThreatened.replaceAll("eq:", "")); // eq:true to true, etc.
        }

        if (StringUtils.isNotBlank(isInvasive)) {
            speciesListQuery.setIsInvasive(
                    isInvasive.replaceAll("eq:", "")); // eq:true to true, etc.
        }
    ...
    /**
     * Convert the results to a version 1 legacy format Note: this is not 100% backwards compatible
     * with the old API, see the
     *
        ...
        try {
            if (Boolean.TRUE.equals(nonulls)) {
                return ResponseEntity.badRequest()
                        .body("The 'nonulls' parameter is not yet supported.");
            }

            String effectiveOrder =
                    StringUtils.isNotBlank(order)
                            ? order
                            : (StringUtils.isNotBlank(dir) ? dir : "asc");
            ...
            if (effectiveOffset != 0 && (effectiveOffset % effectiveMax) != 0) {
                return ResponseEntity.badRequest()
                        .body(
                                String.format(
                                        "Invalid pagination: 'offset' (%d) must be a multiple of 'max' (%d).",
                                        effectiveOffset, effectiveMax));
            }

            boolean hasListFilters =
                    StringUtils.isNotBlank(isAuthoritative)
            ...
            if (hasListFilters || StringUtils.isBlank(druid)) {
                ...
                Page<SpeciesList> speciesLists =
                        searchHelperService.searchDocuments(
                                convertedSpeciesListQuery, userId, isAdmin, decodedQuery, paging);
                ...
                String speciesListIDs =
                        speciesLists.getContent().stream()
                                .map(
                                        list ->
                                                list.getDataResourceUid() != null
                ...
                return new ResponseEntity<>(
                        legacyService.convertListItemToVersion1(speciesListItems), HttpStatus.OK);
            }
            // Branch B: Direct ID Lookup
            else {
                return getLegacySpeciesListItems(
                        druid,
                        decodedQuery,
                        null,
                        nonulls,
    ...
    @Operation(
            tags = "REST v1",
            summary = "Search species list items",
            deprecated = true,
            description =
                    "Search across items using filters. Use 'druid' as a query parameter here.")
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
                description = "Species lists items found",
                content =
                        @Content(
                                mediaType = MediaType.APPLICATION_JSON_VALUE,
    ...
    public ResponseEntity<Object> searchSpeciesListItems(
            @Parameter(
                            description = "Filter by list ID (druid) as query parameter",
                            example = "dr656")
                    @RequestParam(name = "druid", required = false)
                    String druid,
            ...
            @Parameter(
                            description = "Query string to find items within the specified list",
                            example = "Eucalyptus")
        ...
        return internalSpeciesListItems(
                druid,
                isAuthoritative,
                isThreatened,
                isInvasive,
    ...
    @Operation(
            tags = "REST v1",
            summary = "Get items by specific List ID",
            deprecated = true,
    ...
    public ResponseEntity<Object> getSpeciesListItemsByPath(
        ...
        return internalSpeciesListItems(
                druid, null, null, null, null, null, query, null, sort, order, null, max, offset,
                principal);
    }
    ...
    @Operation(
            tags = "REST v1",
            summary = "Get species list metadata for a given species list ID",
            deprecated = true)
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
    ...
    @Operation(
            tags = "REST v1",
            summary = "(Internal use) Get  species list metadata for a given species list ID",
            deprecated = true,
            hidden = false)
    @ApiResponses({
        @ApiResponse(
    ...
    @GetMapping({
        "/v1/speciesListInternal/{speciesListID}",
        "/v1/speciesListInternal/{speciesListID}/"
    })
        ...
        if (!authUtils.isAuthorized(principal)) {
            ErrorResponse errorResponse =
                    new ErrorResponse(
                            ...
                            "Not authorised to access this endpoint",
    ...
    /**
     * Get the details of a species list by its ID or data resource UID. This method is used by both
     * the public and internal endpoints.
     *
     * @param speciesListID
     * @return
     */
    private ResponseEntity<Object> getListDetails(
            String speciesListID, @AuthenticationPrincipal Principal principal) {
    ...
    @Operation(
            tags = "REST v1",
            summary = "Get species list items (internal use) for a given species list ID",
            deprecated = true)
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
    ...
    public ResponseEntity<Object> speciesListInternal(
            @Parameter(
                            name = "druid",
                            description =
                                    "The data resource id (or speciesListID) or comma separated ids to identify list(s) to return list items for e.g. '/v1/speciesListItemsInternal/dr123,dr456,dr789'",
            ...
            @Parameter(
                            name = "includeKVP",
                            description =
                                    "Whether to include KVP (key value pairs) values in the returned list item. Note this is now ignored and  KVP values are always returned.",
                            schema = @Schema(type = "boolean", defaultValue = "true"))
                    @RequestParam(name = "includeKVP", defaultValue = "false")
                    Boolean _includeKVP,
        ...
        try {
            ...
            return getLegacySpeciesListItems(
                    speciesListIDs, searchQuery, null, nonulls, offset, max, sort, dir, principal);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
    }
     ...
     * @return
     */
    private ResponseEntity<Object> getLegacySpeciesListItems(
            String speciesListIDs,
            String searchQuery,
            String fields,
            Boolean nonulls,
        ...
        logger.debug(
                "Fetching legacy species list items for speciesListIDs: {} with offset: {}, max: {}",
                speciesListIDs,
                offset,
                max);
        logger.debug(
                "Calculated page and pageSize: {} with page: {}, pageSize: {}",
                speciesListIDs,
                page,
                pageSize);
        List<SpeciesListItem> speciesListItems;
        ...
        List<SpeciesListItemVersion1> legacySpeciesListItems =
                legacyService.convertListItemToVersion1(speciesListItems);
    ...
    @GetMapping({
        "/v1/listCommonKeys/{speciesListIDs}",
        "/v1/listCommonKeys/{speciesListIDs}/",
        "/v1/listCommonKeys",
        "/v1/listCommonKeys/"
    })
    public ResponseEntity<Object> speciesListCommonKeys(
            @Parameter(
                            ...
                            example = "dr18404,dr18457",
                            schema = @Schema(type = "string"))
                    @PathVariable(value = "speciesListIDs", required = false)
                    String speciesListIDs,
            @Parameter(
                            name = "druid",
                            description =
                                    "List of species list IDs (comma-separated), alternative to path variable",
                            example = "dr10637")
                    @RequestParam(value = "druid", required = false)
                    String druid,
            @AuthenticationPrincipal Principal principal) {
        if (speciesListIDs != null && druid != null) {
            return ResponseEntity.badRequest()
                    .body(
                            "Provide either speciesListIDs as a path variable or druid as a query parameter, not both.");
        }
        if (speciesListIDs == null) speciesListIDs = druid;
        if (speciesListIDs == null) {
            return ResponseEntity.badRequest()
                    .body("speciesListIDs or druid parameter is required");
        }
        try {
            ...
            if (!speciesLists.isEmpty()) {
                ...
                // Perform string replacement for some values in the Set based on a Map<String,
                // String>
    ...
    @Operation(
            tags = "REST v1",
            summary = "Get a list of species lists metadata by (taxon) GUID/s",
            deprecated = true)
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
    ...
    public ResponseEntity<Object> speciesListItemsForGuid(
            @Parameter(
                            name = "guids",
                            description =
                                    "Species GUIDs (can be comma-separated) search on. Note this can be provided as a URL path for backwards compatibility.",
                            example =
            ...
            @Parameter(
                            name = "speciesListIDs",
                            description =
                                    "Optional list of species list IDs (can be comma-separated) to filter the results.",
                            example = "dr18404,dr18457",
                            schema = @Schema(type = "string"))
                    @Nullable
            ...
            @Nullable @RequestParam(name = "pageSize", defaultValue = "9999") @Max(10000)
                    Integer pageSize,
            @RequestParam(name = "isAuthoritative", required = false) String isAuthoritative,
            @RequestParam(name = "isThreatened", required = false) String isThreatened,
            @RequestParam(name = "isInvasive", required = false) String isInvasive,
        ...
        String guid =
                (fullUrl.split("/v1/species/").length > 1) ? fullUrl.split("/v1/species/")[1] : "";
        ...
        int pageSizeVal =
                Math.max((pageSize != null ? pageSize : 9999), 1); // Ensure pageSize is at least 1

        String inputGuids =
                (StringUtils.isNotBlank(guid)
                        ? guid
                        : (StringUtils.isNotBlank(guids) ? guids : ""));
        ...
        try {
            ...
            if (hasBooleanFilters) {
                ...
                Page<SpeciesList> matchingLists =
                        searchHelperService.searchDocuments(
                                speciesListQuery.convertTo(),
                                userId,
                                isAdmin,
                                ".*",
                                PageRequest.of(0, 10000));

                String filteredListIDs =
                        matchingLists.getContent().stream()
                                .map(
                                        list ->
                                                list.getDataResourceUid() != null
                                                        ? list.getDataResourceUid()
                ...
                if (StringUtils.isNotBlank(speciesListIDs)) {
                    Set<String> callerIDs =
                            new java.util.HashSet<>(Arrays.asList(speciesListIDs.split(",")));
                    filteredListIDs =
                            Arrays.stream(filteredListIDs.split(","))
            ...
            List<SpeciesItemVersion1> legacySpeciesListItems =
                    legacyService.convertToSpeciesItemVersion1(speciesListItems);
    ...
    @Operation(
            ...
            description =
                    "Get details of individual items i.e. species for specified species list(s) filter-able by specified fields",
            deprecated = true)
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
    ...
    public ResponseEntity<Object> queryListItemOrKVP(
            ...
            @Nullable @RequestParam(name = "fields")
                    String fields, // not yet implemented in service
        ...
        try {
            ...
            List<SpeciesListItem> speciesListItems =
                    searchHelperService.fetchSpeciesListItems(
                            druid, q, fields, null, page, pageSize, sort, order, principal);

            if (speciesListItems.isEmpty()) {
                return new ResponseEntity<>(new ArrayList<>(), HttpStatus.OK);
            }

            List<QueryListItemVersion1> legacySpeciesListItems =
                    legacyService.convertQueryListItemToVersion1(speciesListItems);
    ...
    /**
     * Calculate the page and size for Spring Data Pageable pagination from legacy offset and max
     * parameters.
     *
    ...
    private static int[] calculatePageAndSize(@Nullable Integer offset, @Nullable Integer max) {
        int page =
                ((offset != null ? offset : 0)
                        / (max != null
                                ? max
                                : 10)); // was + 1 for 1-based page, but Pageable is zero-based
