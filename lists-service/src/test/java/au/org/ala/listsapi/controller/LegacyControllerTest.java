...
class LegacyControllerTest {
  ...
  @Mock private SpeciesListLegacyService legacyService;

  @Mock private AuthUtils authUtils;

  @Mock private Principal principal;

  @InjectMocks private LegacyController legacyController;

  @Captor private ArgumentCaptor<SpeciesList> speciesListCaptor;
  ...
  void speciesListSearch_withListType_parsesAndPassesListType() {
    ...
    // Assert
    verify(searchHelperService)
        .searchDocuments(speciesListCaptor.capture(), any(), any(), anyString(), any());
    SpeciesList capturedQuery = speciesListCaptor.getValue();

    assertEquals(
        "PROFILE",
        capturedQuery.getListType(),
        "ListType should be parsed correctly, removing 'eq:'");
  }
}
